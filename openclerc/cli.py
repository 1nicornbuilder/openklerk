"""
OpenClerc CLI -- Command-line interface for the filing engine.

Usage:
    openclerc run --config entity.json --filer ca_soi
    openclerc new-filer --state Oregon --filing-type "Annual Report"
    openclerc check --filer california_soi
    openclerc analyze --screenshots ./screenshots/oregon/
    openclerc post --demo-video ./demo.mp4
    openclerc list
    openclerc --version
"""
import asyncio
...
@click.group()
@click.version_option(version=openclerc.__version__, prog_name="openclerc")
def cli():
    """OpenClerc -- Open-source government compliance filing engine."""
    pass
...
def run(config, filer, headless, screenshots_dir, output, mock_llm):
    ...
    from openclerc.filers import get_filer_for_code
    ...
    if not filer_instance:
        ...
        console.print("Run 'openclerc list' to see available filers.")
        sys.exit(1)
    ...
    openklerk = None
    if mock_llm:
        from openclerc.intelligence.service import MockOpenKlerkService
...
def new_filer(state, filing_type):
    ...
    from openclerc.contrib.scaffold import create_filer_scaffold
...
def check(filer):
    ...
    from openclerc.contrib.quality_gate import run_quality_checks
...
def analyze(screenshots, state, mock_llm):
    ...
    from openclerc.intelligence.analyzer import analyze_screenshots
...
def post(demo_video, state, filing_type, duration, open_linkedin, mock_llm):
    ...
    from openclerc.demo.viral_post import generate_post
...
def list_filers():
    ...
    from openclerc.filers import list_registered_filers
