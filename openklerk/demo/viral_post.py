...
async def generate_post(
    ...
    if openklerk_service:
        post_text = await openklerk_service.generate_text(
            ...
            f"{filing_type} module for {state} to OpenKlerk. Demo shows AI "
            f"filing in {duration} seconds. Hook -- mention OpenKlerk + NeverMissAFiling "
...
def _default_post(state: str, filing_type: str, duration: int) -> str:
    return f"""Just contributed a {filing_type} module for {state} to OpenKlerk!

Watch the AI file a {filing_type} in {duration} seconds -- fully autonomous, zero human intervention.

OpenKlerk is the open-source engine behind @NeverMissAFiling. Every contributor adds a state, records a demo, and helps businesses never miss a filing deadline again.

Want to add YOUR state? It's easier than you think:
1. Take screenshots of your state's filing portal
2. Run `openklerk analyze --screenshots ./your_state/`
3. Fill in the generated code
4. Submit a PR

GitHub: github.com/NeverMissAFiling/openklerk

#OpenKlerk #GovTech #OpenSource"""
